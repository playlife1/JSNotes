...

{
    ...
    console.log(a + ',' + b); //输出结果：playlife,undefined
}
...
正确答案的打印结果是 playlife。因为 `obj` 这个对象并不产生作用域， `sayHello()` 这个箭头函数实际仍然是定义在 window 当中的，所以 这里的 this指向是 window。
...
ES6 中，我们有了扩展运算符，就不用担心报错的问题了。代码可以这样写：
